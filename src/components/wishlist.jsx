      ...
      if (token) {
        try {
          const response = await axios.get(
            `/user/wishlist/view?token=${token}`
          );
    ...
    if (token) {
      try {
        const response = await axios.put(
          `/user/wishlist/remove?token=${token}`,
          {
            productId: id,
          }
        );
          ...
          {loadingCart ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {cart.map((item) => (
                <div
                  key={item._id}
                  className="bg-white p-4 mt-10 rounded-lg shadow-md relative"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {loadingProducts ? (
                  ...
                  recommendedProducts.map((product) => (
                    <div
                      key={product._id}
                      className="bg-white cursor-pointer p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow"
                    >
                      <img
