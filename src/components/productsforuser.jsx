...
const Productsforuser = () => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            ...
            try {
                const response = await axios.get(`/users/user/currentuser?token=${token}`);
    ...
    return (
        <div className="flex flex-col min-h-screen bg-gray-100">
            ...
            <div className="flex flex-col lg:flex-row mt-4 ml-4 lg:mb-0">
                {/* Sidebar Filter */}
                <aside className={`w-[85vw] lg:w-64  lg:h-[80vh] mt-8 ml-2  p-4 mb-4 lg:mb-0 lg:sticky lg:top-0 ${isCategoriesVisible ? 'h-[74vh] lg:w-64' : 'h-[5vh] lg:w-3'}`}>
            {/* Categories */}
            <h2 
...
>
    ...
    {isCategoriesVisible ? (
        <FaAngleUp size={24} className="text-gray-800 transition-transform duration-300" />
    ) : (
        <FaAngleDown size={24} className="text-gray-800 transition-transform duration-300" />
    )}
</h2>
                    ...
                    {categories.map((category) => (
                        <li key={category}>
                            <button
                                ...
                                className={`w-full p-2 rounded-lg text-left ${selectedCategory === category ? 'bg-black text-white' : ' text-gray-700'} hover:bg-black hover:text-white transition duration-300`}
                            >
                                {category}
                            </button>
                        </li>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6">
                        {noProductsFound ? (
                            ...
                            products.map(product => (
                                ...
                                >
                                    {/* Product Image */}
                                    <div onClick={() => setSelectedProduct(product)} className="w-full h-56 cursor-pointer flex justify-center items-center overflow-hidden rounded-t-lg">
                                        <img
                                    ...
                                    <div onClick={() => navigate(`/singleproduct/${product._id}`)} className="p-4 cursor-pointer">
  ...
  <div className="text-lg font-light text-black">
    ...
    <span className="text-lg text-black">Rs. {product.priceAfterDiscount}</span>
                                    ...
                                    <div className="absolute z-[999] top-2 right-2 flex space-x-2">
                                        {token && (
                                            <div
                                                ...
                                                className={`w-8 h-8 rounded-full border border-black cursor-pointer flex justify-center items-center hover:bg-gray-100 transition ${loginuser?.wishlist?.indexOf(product?._id) === -1 ? 'opacity-50' : 'opacity-100'}`}
                                            >
                                                <FaHeart
                                                    className={`text-black hover:text-red-500 transition ${loginuser?.wishlist?.indexOf(product?._id) === -1 ? 'text-gray-800' : 'text-red-500'}`}
                                                />
                                            </div>
                                        )}
                ...
                ${currentPage === index + 1 ? ' text-black font-semibold' : ' text-gray-700  hover:bg-gray-300'}`}
                            >
                                {index + 1}
                            </button>
                        ))}
