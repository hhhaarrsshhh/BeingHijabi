...
const ProductsPage = () => {
  ...
  const handleDelete = async (productId) => {
    ...
    if (token) {
      try {
        await axios.delete(
          `/products/product/delete/${productId}?token=${token}`
        );
      ...
      <div className="products-page p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {products.map((product) => (
            <div
              key={product._id}
              className="product-card bg-white border rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow duration-300"
            >
              <img
