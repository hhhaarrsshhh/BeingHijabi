...
const ViewCart = () => {
    ...
    const handleRemove = async (id) => {
        ...
        if (token) {
            try {
                const response = await axios.put(`/users/user/cart/remove?token=${token}`, { productId: id });
    ...
    const calculateTotal = () => {
        return cart.reduce((total, item) => total + (item.priceAfterDiscount - item.discount) * item.quantity, 0).toFixed(2);
    };
    ...
    return (
        <>
            ...
            <button 
                    ...
                    {loadingCart ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {cart && cart.length > 0 && cart.map(item => (
                                <div key={item._id} className="relative flex flex-col items-center justify-center border border-gray-200 p-4 shadow-md rounded-lg bg-white">
                                    {/* Close Button */}
                                    <button
                                        ...
                                        <div className="flex items-center justify-center mt-2">
                                            ...
                                            <div className="flex items-center space-x-1">
                                                <button
                                                    ...
                                                    className={`w-6 h-6 flex justify-center items-center bg-gray-200 rounded-full ${item.quantity <= 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-300'}`}
                ...
                {/* Right Side - Summary */}
                {cart.length > 0 && (<div className="lg:w-1/3 lg:h-72 bg-white p-6  shadow-md lg:sticky lg:top-0  mt-10 rounded-lg">
