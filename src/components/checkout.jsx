...
import axios from '../utils/axios'; // Import axios to make API calls
...
const Checkout = () => {
  ...
  const [newAddress, setNewAddress] = useState({
    ...
    city: '',
    state: '',
    postalCode: '', // Initialized correctly
    country: '',    // Initialized correctly
    phoneNumber: ''
  ...
  const handlePlaceOrder = () => {
    ...
    // Razorpay options
    const options = {
      key: import.meta.env.VITE_RAZORPAY_KEY_ID, // Ensure the correct Vite environment variable is used
      amount: cartDetails.totalAmount * 100, // Amount in smallest currency unit (paise for INR)
      currency: "INR",
      name: 'Being hijabi',
      description: 'Order Payment',
      handler: async function (response) {
        ...
        try {
          ...
          const orderConfirmation = await axios.post(`/order/create?token=${token}`, {
            products: cartDetails.items.map((item) => ({
              ...
              quantity: item.quantity || 1,
              _id: item._id || '', // Assuming item has an ID
            })),
            deliveryAddress: {
              _id: selectedAddress._id || '', // Make sure selectedAddress has an ID
      ...
      prefill: {
        email: profile.email,
        contact: selectedAddress.phoneNumber || '', // Ensure phone number is valid
      },
      theme: {
        color: '#3399cc',
      },
