...
const ProductGrid = () => {
      ...
      <div className="relative flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow-md p-4 transition transform hover:scale-105">
        ...
        >
          <img
            src={product.image}
            alt={product.name}
            className={`w-full h-48 object-cover rounded-md transition-all duration-600 ${showView ? "scale-110" : ""}`}
          />
          {showView && (
        ...
        <div className="mt-4 text-left space-y-1 mr-16">
          ...
          <div className="flex items-center mt-3 justify-center space-x-2 border border-gray-300 hover:border-black rounded p-2">
            <button className="px-3 py-1 bg-gray-200 rounded focus:outline-none hover:border-black">-</button>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* Load More button, conditionally rendered if there are more products to load */}
